  ...
  async handleCron() {
    ...
    for (const recurrence of activeRecurrences) {
      try {
        ...
        if (
          recurrence.dataFim &&
          isBefore(new Date(recurrence.dataFim), nextDate)
        ) {
          // Expired
        ...
        if (
          isBefore(lastDateStart, today) ||
          lastDateStart.getTime() === today.getTime()
        ) {
          // Create Next Transaction
          const { id, createdAt, updatedAt, dataPagamento, ...dataToCopy } =
            lastTransaction;
          ...
          this.logger.log(
            `Generated recurring transaction for recurrence ${recurrence.id} on ${nextDate}`,
          );
        }
      } catch (e) {
