...
async function main() {
  ...
  try {
    ...
    console.log(
      `Enriched returned ${resEnriched.data.transacoes?.length || 0} items.`,
    );
    if (resEnriched.data.transacoes?.length > 0) {
      console.log(
        'Sample Enriched Item (First):',
        JSON.stringify(resEnriched.data.transacoes[0], null, 2),
      );
      console.log(
        'Sample Enriched Item (Last):',
        JSON.stringify(resEnriched.data.transacoes.slice(-1)[0], null, 2),
      );
    }
  } catch (e: any) {
  ...
  console.log(
    `Standard returned ${resStd.data.transacoes?.length || 0} items.`,
  );
