...
async function main() {
    ...
    console.log(`Standard Extract (Jan 26-28) count: ${resStd.data.transacoes?.length || 0}`);
    ...
    const recent = resMonth.data.transacoes?.filter((t: any) => t.dataEntrada >= '2026-01-25') || [];
    console.log(`Total transactions in Jan: ${resMonth.data.transacoes?.length || 0}`);
    console.log(`Transactions on/after Jan 25: ${recent.length}`);
    if (recent.length > 0) {
        console.log('Details of items on/after 25th:', JSON.stringify(recent, null, 2));
    }
