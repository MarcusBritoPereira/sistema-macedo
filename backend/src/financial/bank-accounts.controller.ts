  ...
  async findAll() {
    ...
    const [lastStatements, reconciledAgg] = await Promise.all([
      this.prisma.extratoBancario.findMany({
        ...
        orderBy: { data: 'desc' },
        select: { data: true, importacao: { select: { contaBancariaId: true } } },
      }),
