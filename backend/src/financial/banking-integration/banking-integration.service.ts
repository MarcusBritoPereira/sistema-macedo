...
export class BankingIntegrationService {
    ...
    async configure(dto: ConfigureBankingDto, files?: { certificate?: any[], privateKey?: any[] }) {
            ...
            if (!accountId) {
                ...
                if (existing) {
                    if (existing.integracao) {
                        throw new BadRequestException('Esta conta já possui uma integração vinculada.');
                    }
                    console.log('Service: Found existing account (unlinked):', existing.id);
                ...
                console.log('Service: Updating provided accountId identification fields:', accountId);
    ...
    async syncStatements(contaBancariaId: string) {
        ...
        try {
            ...
            while (hasMore) {
                log(`[Sync] Fetching page ${currentPage} (Window: ${dataInicio} to ${dataFim})...`);
                ...
                if (!responseData) throw new Error('API do banco não respondeu após várias tentativas.');
                ...
                const pageSize = 1000;
                if (pageTrans.length < pageSize || (responseData.totalPaginas !== undefined && currentPage >= responseData.totalPaginas - 1)) {
                    hasMore = false;
                } else {
                    currentPage++;
            ...
            log(`[Sync] Fetched total of ${allStatements.length} transactions across ${currentPage + 1} pages.`);
            ...
            log(`[Sync] Import done: ${importedCount} imported, ${duplicatesCount} duplicates.`);
            ...
            if (error.message && (error.message.includes('autenticação') || error.message.includes('certificado'))) {
                await this.prisma.integracaoBancaria.update({
                    where: { id: integration.id },
                    data: { status: 'ERROR' }
                });
