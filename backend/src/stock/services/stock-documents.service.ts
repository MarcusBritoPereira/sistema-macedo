...
export class StockDocumentsService {
  ...
  async create(
      ...
      dto.items.map(async (item) => {
        ...
        if (!material || !material.ativo)
          throw new BadRequestException(
            'Material ativo não encontrado no documento',
          );
        ...
        const custoUnitario = item.custoUnitario
          ? new Prisma.Decimal(item.custoUnitario)
    ...
    const valorTotal = items.reduce(
      (acc, item) => acc.plus(item.custoTotal),
      new Prisma.Decimal(0),
    );
  ...
  async findOne(id: string) {
    ...
    if (!document)
      throw new NotFoundException('Documento de estoque não encontrado');
    return document;
  }
  ...
  async submit(id: string, userId: string) {
    ...
    await this.audit(
      userId,
      'ESTOQUE_DOCUMENTO_ENVIADO_APROVACAO',
      id,
      document,
      updated,
    );
  ...
  async approve(id: string, userId: string) {
    ...
    if (
      document.status !== StatusDocumentoEstoque.RASCUNHO &&
      document.status !== StatusDocumentoEstoque.PENDENTE_APROVACAO
    ) {
      throw new BadRequestException(
        'Documento não pode ser aprovado no status atual',
      );
    }
    const updated = await this.prisma.documentoEstoque.update({
      where: { id },
  ...
  async post(id: string, userId: string) {
    ...
    if (
      document.status !== StatusDocumentoEstoque.RASCUNHO &&
      document.status !== StatusDocumentoEstoque.APROVADO
    ) {
      throw new BadRequestException(
        'Documento não pode ser efetivado no status atual',
      );
    }
    if (!document.itens.length)
      throw new BadRequestException(
        'Documento sem itens não pode ser efetivado',
      );

    const transacaoFinanceiraId =
      document.tipo === TipoDocumentoEstoque.ENTRADA
        ? await this.financialIntegration.ensurePurchaseTransaction(
            document.id,
            userId,
          )
    ...
    const updated = await this.prisma.documentoEstoque.update({
      where: { id },
      data: {
        status: StatusDocumentoEstoque.EFETIVADO,
        efetivadoPorId: userId,
      },
      include: this.includeRelations(),
    });
    await this.audit(
      userId,
      'ESTOQUE_DOCUMENTO_EFETIVADO',
      id,
      document,
      updated,
    );
  ...
  async cancel(id: string, userId: string, motivo: string) {
    ...
    if (document.status === StatusDocumentoEstoque.EFETIVADO) {
      ...
      for (const movement of document.movimentos) {
        reversed.push(
          await this.movementService.reverse(movement.id, userId, motivo),
        );
      }
      const updated = await this.prisma.documentoEstoque.update({
        where: { id },
    ...
    if (
      document.status === StatusDocumentoEstoque.CANCELADO ||
      document.status === StatusDocumentoEstoque.ESTORNADO
    ) {
  ...
  private validateDocumentShape(
    tipo: TipoDocumentoEstoque,
    dto: CreateStockDocumentDto,
  ) {
    if (tipo === TipoDocumentoEstoque.ENTRADA && !dto.localDestinoId)
      throw new BadRequestException('Entrada exige local de destino');
    if (tipo === TipoDocumentoEstoque.SAIDA && !dto.localOrigemId)
  ...
  private mapMovementType(tipo: TipoDocumentoEstoque) {
    if (tipo === TipoDocumentoEstoque.ENTRADA)
      return TipoMovimentoEstoque.ENTRADA_COMPRA;
    if (tipo === TipoDocumentoEstoque.SAIDA)
      return TipoMovimentoEstoque.SAIDA_CONSUMO;
    if (tipo === TipoDocumentoEstoque.TRANSFERENCIA)
      return TipoMovimentoEstoque.TRANSFERENCIA;
    throw new BadRequestException(
      'Tipo de documento ainda não suportado para efetivação',
    );
  }
  ...
  private async ensureMutable(id: string) {
    ...
    if (document.status !== StatusDocumentoEstoque.RASCUNHO) {
      throw new BadRequestException(
        'Documento não pode ser alterado no status atual',
      );
    }
    return document;
  }
  ...
  private async nextDocumentNumber(tipo: TipoDocumentoEstoque) {
    ...
    const prefix =
      tipo === TipoDocumentoEstoque.ENTRADA
        ? 'ENT'
        : tipo === TipoDocumentoEstoque.SAIDA
          ? 'SAI'
          : 'TRA';
