...
export class StockRequestsService {
  ...
  async approve(id: string, dto: ApproveStockRequestDto, userId: string) {
    ...
    if (
      request.status !== StatusSolicitacaoMaterial.ENVIADA &&
      request.status !== StatusSolicitacaoMaterial.EM_ANALISE &&
      request.status !== StatusSolicitacaoMaterial.RASCUNHO
    ) {
      throw new BadRequestException(
        'Solicitação não pode ser aprovada no status atual',
      );
    }
    const approvals = new Map(
      dto.items.map((item) => [
        item.itemId,
        new Prisma.Decimal(item.quantidadeAprovada),
      ]),
    );
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const item of request.itens) {
        ...
        if (approved.gt(item.quantidadeSolicitada))
          throw new BadRequestException(
            'Quantidade aprovada não pode exceder solicitada',
          );
      ...
      const updated = await tx.solicitacaoMaterial.update({
        ...
        data: {
          status,
          aprovadorId: userId,
          observacao: dto.observacao || request.observacao,
        },
        include: this.includeRelations(),
      });
  ...
  async reject(id: string, motivo: string, userId: string) {
    ...
    if (
      current.status !== StatusSolicitacaoMaterial.RASCUNHO &&
      current.status !== StatusSolicitacaoMaterial.ENVIADA &&
      current.status !== StatusSolicitacaoMaterial.EM_ANALISE
    ) {
      throw new BadRequestException(
    ...
    const updated = await this.prisma.solicitacaoMaterial.update({
      where: { id },
      data: {
        status: StatusSolicitacaoMaterial.REJEITADA,
        aprovadorId: userId,
        observacao: motivo,
      },
      include: this.includeRelations(),
  ...
  async fulfill(id: string, dto: FulfillStockRequestDto, userId: string) {
    const request = await this.findOne(id);
    if (
      request.status !== StatusSolicitacaoMaterial.APROVADA &&
      request.status !== StatusSolicitacaoMaterial.PARCIALMENTE_APROVADA &&
      request.status !== StatusSolicitacaoMaterial.SEPARACAO
    ) {
      throw new BadRequestException(
        'Solicitação não pode ser atendida no status atual',
      );
    }
    ...
    for (const item of request.itens) {
      ...
      const reservedForRequest = await this.getApprovedReservationQuantity(
        request.id,
        request.obraId,
        item.materialId,
        dto.localOrigemId,
    ...
    const refreshed = await this.prisma.solicitacaoMaterial.findUnique({
      where: { id },
      include: { itens: true },
    });
    const allFulfilled = refreshed?.itens.every((item) =>
      (item.quantidadeAprovada || new Prisma.Decimal(0)).lte(
    ...
    const updated = await this.prisma.solicitacaoMaterial.update({
      ...
      data: {
        status: allFulfilled
          ...
          : StatusSolicitacaoMaterial.PARCIALMENTE_ATENDIDA,
      },
      include: this.includeRelations(),
    });
    await this.audit(
      userId,
      'ESTOQUE_SOLICITACAO_ATENDIDA',
      id,
      request,
      updated,
    );
  ...
  async cancel(id: string, userId: string) {
    ...
    if (
      current.status === StatusSolicitacaoMaterial.CANCELADA ||
      current.status === StatusSolicitacaoMaterial.ATENDIDA
    ) {
      throw new BadRequestException(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const reservation of reservations) {
        const key =
          `${reservation.materialId}:` + `${reservation.localEstoqueId}`;

        const existing = grouped.get(key);

        if (existing) {
          existing.quantidade = existing.quantidade.plus(
          ...
          grouped.set(key, {
            ...
            quantidade: new Prisma.Decimal(reservation.quantidade),
          });
        }
      }
      ...
      for (const group of grouped.values()) {
        ...
        if (!saldo) {
          throw new NotFoundException('Saldo da reserva não encontrado');
        }

        const reservadoAtual = new Prisma.Decimal(saldo.quantidadeReservada);
      ...
      const updated = await tx.solicitacaoMaterial.update({
        where: { id },
        data: {
          status: StatusSolicitacaoMaterial.CANCELADA,
        },
        include: this.includeRelations(),
      });
      ...
      await tx.logAuditoria.create({
        data: {
          ...
          tabela: 'solicitacoes_material',
          registroId: id,
          valorAntigo: stringifyAudit(current),
          valorNovo: stringifyAudit(updated),
  ...
  private async getApprovedReservationQuantity(
    ...
    return reservations.reduce(
      (total, reservation) => total.plus(reservation.quantidade),
  ...
  private async releaseReservations(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const reservation of reservations) {
        ...
        const reserved = new Prisma.Decimal(reservation.quantidade);
        ...
        remaining = remaining.minus(reserved);
        ...
        await tx.reservaEstoque.update({
          ...
          data: {
            status: StatusReservaEstoque.ATENDIDA,
          },
        });
      }
      ...
      if (!saldo) {
        throw new NotFoundException('Saldo reservado não encontrado');
      }

      const reservadoAtual = new Prisma.Decimal(saldo.quantidadeReservada);

      if (reservadoAtual.lt(quantity)) {
        throw new BadRequestException('Saldo reservado inconsistente');
      }
  ...
  private includeRelations() {
    return {
      ...
      itens: {
        ...
        orderBy: { material: { nome: 'asc' } } as any,
      },
    };
  }
  ...
  private async ensureActiveProject(obraId: string) {
    ...
    if (!obra || !obra.ativo)
      throw new BadRequestException('Obra ativa não encontrada');
  }
