...
export class CategoryDetailPage implements OnInit {
  ...
  loadCategory(id: string) {
    this.categoriesService.findOne(id).subscribe({
      next: (category) => {
        this.category = {
          ...category,
          subcategorias: category.subcategorias || category.children || [],
        };
      },
      error: (err) => {
        console.error(err);
  ...
  private buildCategoryPayload(): Category {
    return {
      nome: this.category.nome?.trim() || '',
      tipo: this.category.tipo,
      descricao: this.category.descricao?.trim() || undefined,
      classificacao: this.category.classificacao || undefined,
  ...
  async save() {
    ...
    if (this.isNew) {
      this.categoriesService.create(this.buildCategoryPayload()).subscribe({
        next: async () => {
      ...
      if (this.category.id) {
        this.categoriesService
          .update(this.category.id, this.buildCategoryPayload())
          .subscribe({
            next: async () => {
              await loading.dismiss();
              this.router.navigate(['/financial/categories']);
  ...
  async showToast(msg: string | string[], color: string = 'success') {
      ...
      buttons: [
        { text: 'Cancelar', role: 'cancel' },
        {
          ...
          handler: (data) => {
            if (data.nome && data.nome.trim() !== '') {
              this.categoriesService
                .updateSubcategory(sub.id, data.nome, data.descricao)
                .subscribe({
                  next: () => this.loadCategory(this.category.id!),
                  error: (err) => {
                    console.error(err);
  ...
  dreOptions = [
    ...
    {
      value: 'CUSTO_SERVICOS_PRESTADOS',
