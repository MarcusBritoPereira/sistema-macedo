...
import { IonContent, IonHeader, IonTitle, IonToolbar, IonButtons, IonMenuButton, IonBackButton, IonSearchbar, IonSegment, IonSegmentButton, IonLabel, IonList, IonCard, IonCardContent, IonBadge, IonIcon, IonButton, IonChip, ToastController, LoadingController, IonRefresher, IonRefresherContent, IonFab, IonFabButton, ActionSheetController, IonCheckbox } from '@ionic/angular/standalone';
...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, RouterModule, IonContent, IonHeader, IonTitle, IonToolbar, IonButtons, IonMenuButton, IonBackButton, IonSearchbar, IonSegment, IonSegmentButton, IonLabel, IonList, IonCard, IonCardContent, IonBadge, IonIcon, IonButton, IonChip, IonRefresher, IonRefresherContent, IonFab, IonFabButton, IonCheckbox]
})
export class PayablesListPage implements OnInit {
    ...
    // --- Bulk CSV Export ---
    exportCSV() {
        const headers = ['Vencimento', 'Pagamento', 'Descrição', 'Fornecedor', 'Valor Total', 'Valor Pago', 'Situação'];

        const rows = this.displayedItems.map(item => {
            const isPaid = item.status === 'REALIZADO' || item.status === 'CONCILIADO';
            return [
                `"${item.dataVencimento ? new Date(item.dataVencimento).toISOString().split('T')[0] : ''}"`,
                `"${isPaid && item.dataPagamento ? new Date(item.dataPagamento).toISOString().split('T')[0] : ''}"`,
                `"${item.descricao || ''}"`,
                `"${item.fornecedor?.nomeFantasia || item.fornecedor || ''}"`,
                item.valor || 0,
                isPaid ? (item.valor || 0) : 0,
        ...
        link.setAttribute('download', `contas_a_pagar_${new Date().getTime()}.csv`);
    ...
    downloadCsvTemplate() {
        ...
        const csvContent = headers.join(',') + '\n2025-12-31,Conta de Luz,150.50,PREVISTO,';
        ...
        if (link.download !== undefined) {
            const url = URL.createObjectURL(blob);
            link.setAttribute('href', url);
            link.setAttribute('download', 'modelo_importacao_pagar.csv');
            link.style.visibility = 'hidden';
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
    ...
    processCsvData(csvText: string) {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const t: any = {
                ...
                descricao: cols[1]?.trim() || 'Despesa Importada',
